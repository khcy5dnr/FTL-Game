...
string coloniseGaia(Fleet* fleet, Gaia *gaia);

mutex mtx;           // mutex for critical section

static volatile long long velocity = 0;
...
void startSimulation (int thread_ID, string data) {
    ...
    if(thread_ID == 1){
        ...
        fleet2 = fleet;
        Gaia *gaia = new Gaia();
        if(velocity > fleet->getVelocity()){
            cout << "Both fleets has departed....\n\n\n" << endl;
            simulateAttacks(fleet1, gaia, 1);
            simulateAttacks(fleet2, gaia, 2);

            coloniseGaia(fleet1,gaia);
            coloniseGaia(fleet2,gaia);
            cout << "\n\nWINNER: FLEET " << gaia->getColonyName() << endl;
        }
        else if(velocity < fleet->getVelocity()){
            cout << "Both fleets has departed....\n\n\n" << endl;
            simulateAttacks(fleet2, gaia, 2);
            simulateAttacks(fleet1, gaia, 1);
...
void simulateAttacks(Fleet* fleet, Gaia *gaia, int num){
    ...
    cout << "At year " << (fleet->getYearsToGaia()/2) << ",Fleet " << fleet->getCorporationName() << " was attacked. The report as follows: "<< endl;
    fleet->spaceAttack();
    ...
    cout << "Shortly after space attack,Fleet " << fleet->getCorporationName() << " was hit with a disease outbreak in one of its ships. The report as follows: "<< endl;
...
string coloniseGaia(Fleet* fleet, Gaia *gaia){
    ...
    cout << "\n\nFleet " << fleet->getCorporationName() << " is entering Gaia." << endl;

    if(gaia->getPopulation() == 0){
        cout << "Checking population...\nPlanet Gaia's is uncolonised." << endl;
        cout << "Fleet " << fleet->getCorporationName() << " has colonised Gaia." << endl;

        gaia->setPopulation(fleet->getColonistCount());
        gaia->setColonyName(fleet->getCorporationName());
        ...
        cout << "Gaia's current population: " << gaia->getPopulation() << endl;
    }
    else if(gaia->getPopulation() != 0){
        cout << "Checking population...\nPlanet has been colonised. " << endl;
        if(fleet->getColonistCount() < gaia->getPopulation()){
            cout << "Current Gaia population: " << gaia->getPopulation();
            cout << "   Current Fleet population: " << fleet->getColonistCount() << endl;
            cout << "Planet Gaia's population is higher. Searching for another planet..." << endl;

        }
        else if(fleet->getColonistCount() > gaia->getPopulation()){
            cout << "Current Gaia population: " << gaia->getPopulation();
            cout << "   Current Fleet population: " << fleet->getColonistCount() << endl;
    ...
    return gaia->getColonyName();
}

int main ()
{

    threads.push_back(thread(startSimulation,1,"Fleet1.dat"));
    threads.push_back(thread(startSimulation,2,"Fleet2.dat"));
